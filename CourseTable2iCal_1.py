...
def login(session, init_url):
    """
    步骤：
    1. 由1.tongji.edu.cn进行两次重定向（get）
    2. 获得登录链接（get）
    3. 提交登录信息（post）
    4. 由返回内容判断是否登录成功，如成功返回ids跳转链接
    """
...
def get_course_info(session, init_url):
    ...
    # 获取课表json
    table_url = "https://1.tongji.edu.cn/api/electionservice/reportManagement/findStudentTimetab?calendarId={}&studentCode={}"
...
def process_course(course):
    ...
    for timetable in course["timeTableList"]:
        ...
        dscr = description.format(timetable["teacherName"],
                                  course["credits"],
                                  course["remark"])
